...
function luxeSeekFromSlider(slider) {
  if (!slider || !audio) return false;

  const duration = Number(audio.duration);
  const percentage = Number(slider.value);

  if (!Number.isFinite(duration) || duration <= 0 || !Number.isFinite(percentage)) {
    return false;
  }
...
function luxeBindProgressSlider() {
  ...
  slider.addEventListener("touchmove", event => {
    /* The range input itself receives the horizontal movement. */
    luxeIsSeeking = true;
    event.stopPropagation();
  }, { passive: true });
  ...
  slider.addEventListener("input", event => {
    luxeIsSeeking = true;
    luxeSeekFromSlider(event.currentTarget);
    event.stopPropagation();
  });
