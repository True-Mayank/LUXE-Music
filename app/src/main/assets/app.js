...
/* =========================================================
   LUXE V6 — MINI PLAYER CONTROLS
   These controls work directly from the mini player so the
   user does not need to reopen the full player for basic
   playback actions.
   ========================================================= */

function luxeSyncMiniControls() {
  const song = current >= 0 ? songs[current] : null;
  const favorite = !!song && isFav(song);

  const favoriteBtn = $("#miniFavorite");
  if (favoriteBtn) {
  ...
  const shuffleBtn = $("#miniShuffle");
  if (shuffleBtn) {
    const fullShuffle = $("#shuffleBtn");
    shuffleBtn.classList.toggle("active", !!fullShuffle?.classList.contains("active"));
  }
...
if ($("#miniShuffle")) {
  $("#miniShuffle").addEventListener("click", event => {
    ...
    const fullShuffle = $("#shuffleBtn");
    if (fullShuffle) fullShuffle.click();
    luxeSyncMiniControls();
  });
}
...
if ($("#miniRepeat")) {
  $("#miniRepeat").addEventListener("click", event => {
    ...
    const fullRepeat = $("#repeatBtn");
    if (fullRepeat) fullRepeat.click();
    luxeSyncMiniControls();
  });
}

/* Keep mini controls synchronized whenever the full-player state changes. */
const luxeMiniSyncTimer = setInterval(() => {
