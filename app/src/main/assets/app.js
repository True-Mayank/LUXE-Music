...
let repeat = false;
let playbackOrder = localStorage.getItem("luxe_playback_order") || "consecutive";
...
function playSong(
  ...
  if (playbackOrder === "random" && baseQueue.length > 1) {
    const remaining =
      baseQueue.filter(index => index !== songIndex);

    currentQueue =
      [songIndex, ...shuffleArray(remaining)];
  } else {
...
if ($("#miniPlayer")) {
  ...
  miniPlayer.addEventListener("click", event => {
    ...
    if (control && miniPlayer.contains(control)) {
      return;
    }
...
if (mini) {
  ...
  mini.addEventListener(
    ...
    event => {
      ...
      /* Mini-player buttons must never open the full player. */
      if (event.target.closest("button, input, select, textarea, a")) {
        mini.classList.remove("swiping");
        mini.style.transform = "";
        touchMoved = false;
        return;
      }
  ...
  mini.addEventListener(
    ...
    event => {
      ...
      /* Every mini-player control handles its own click. */
      if (event.target.closest("button, input, select, textarea, a")) {
        event.stopPropagation();
        return;
      }
...
let luxeIsSeeking = false;
let luxeSeekPointerId = null;

function luxeSeekFromSlider(slider, clientX = null) {
  ...
  /*
     When the user touches/clicks the bar, calculate the position
     directly from the bar's screen coordinates. This is more reliable
     than depending only on the browser/WebView range-input behavior.
  */
  ...
  const targetTime = duration * safePercentage / 100;

  try {
    audio.currentTime = targetTime;
  } catch (error) {
    console.error("LUXE seek error:", error);
    return false;
  }

  slider.value = String(safePercentage);
...
function luxeBindProgressSlider() {
  const slider = $("#progress");
  if (!slider || slider.dataset.luxeSeekBound) return;

  slider.dataset.luxeSeekBound = "1";
  slider.min = "0";
  slider.max = "100";
  slider.step = "0.01";

  /* Never let the full-player swipe gesture steal slider touches. */
  ...
  slider.addEventListener("pointerdown", event => {
    ...
    luxeIsSeeking = true;
    luxeSeekPointerId = event.pointerId;

    try {
      slider.setPointerCapture(event.pointerId);
    } catch {}

    luxeSeekFromSlider(slider, event.clientX);
  });

  slider.addEventListener("pointermove", event => {
    if (!luxeIsSeeking) return;
  ...
  /* Fallback for WebViews that don't deliver pointer events correctly. */
  ...
  /* Clicking anywhere on the bar immediately jumps to that position. */
...
/* =========================================================
   PLAYBACK ORDER
   Random / Consecutively
   ========================================================= */

function savePlaybackOrder() {
  localStorage.setItem("luxe_playback_order", playbackOrder);
}

function updatePlaybackOrderUI() {
  const random = $("#orderRandom");
  const consecutive = $("#orderConsecutive");

  random?.classList.toggle("active", playbackOrder === "random");
  consecutive?.classList.toggle("active", playbackOrder === "consecutive");

  const button = $("#orderBtn");
  if (button) {
    button.classList.toggle("active", playbackOrder === "random");
    button.title = playbackOrder === "random"
      ? "Playback order: Random"
      : "Playback order: Consecutively";
    button.setAttribute("aria-label", button.title);
  }
}

function openPlaybackOrder() {
  updatePlaybackOrderUI();
  $("#playOrderPanel")?.classList.remove("hidden");
}

function closePlaybackOrder() {
  $("#playOrderPanel")?.classList.add("hidden");
}

function setPlaybackOrder(mode) {
  if (mode !== "random" && mode !== "consecutive") return;
  ...
  /*
    Rebuild the active queue from the current track so the
    new mode applies immediately to the next song.
  */
  if (current >= 0 && songs.length) {
    const source =
      currentQueue.length
        ? currentQueue.map(index => songs[index]).filter(Boolean)
        : songs;

    const currentSong = songs[current];
    const remaining = source.filter(song => song !== currentSong);

    if (playbackOrder === "random") {
      currentQueue = [
        current,
        ...shuffleArray(remaining.map(song => songs.indexOf(song)).filter(index => index >= 0))
      ];
    } else {
      currentQueue = source
      ...
      if (!currentQueue.includes(current)) currentQueue.unshift(current);
    }

    queuePosition = currentQueue.indexOf(current);
  }
}
...
if ($("#closePlayOrder")) {
  $("#closePlayOrder").addEventListener("click", closePlaybackOrder);
}

if ($("#playOrderPanel")) {
  $("#playOrderPanel").addEventListener("click", event => {
    if (event.target === $("#playOrderPanel")) closePlaybackOrder();
  });
}

if ($("#orderRandom")) {
  $("#orderRandom").addEventListener("click", () => setPlaybackOrder("random"));
}

if ($("#orderConsecutive")) {
  $("#orderConsecutive").addEventListener("click", () => setPlaybackOrder("consecutive"));
}
