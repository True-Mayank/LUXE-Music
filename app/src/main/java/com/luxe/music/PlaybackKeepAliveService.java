...
public class PlaybackKeepAliveService extends Service {
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        // Do not recreate playback if Android removes the LUXE task from
        // Recent Apps. Normal Home/lock-screen backgrounding does not call
        // onTaskRemoved(), so playback continues normally.
        return START_NOT_STICKY;
    }

    @Override
    public void onTaskRemoved(Intent rootIntent) {
        // This callback is invoked when the LUXE app task is explicitly
        // swiped away from Recent Apps. Stop playback only in that case.
        stopPlayback(true);
        stopSelf();
