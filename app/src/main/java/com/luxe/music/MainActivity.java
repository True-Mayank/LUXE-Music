...
public class MainActivity extends Activity {
                ...
                if (AUDIO_HOST.equals(url.getHost()) &&
                    ...
                    return new WebResourceResponse(
                            ...
                            new java.io.ByteArrayInputStream(
                                    "Audio unavailable".getBytes(StandardCharsets.UTF_8)
                            )
                    );
                }
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        // Always persist READ permission. Some document providers do not return
        // a WRITE flag even though the selected folder is perfectly readable.
        try {
            ...
            // The selected provider may not support persistable permissions.
            // We still scan the folder for this session.
        }
    ...
    private void restoreSavedFolder() {
        ...
        try {
            ...
            if (!hasPersistedReadPermission(treeUri)) {
                // The OS/provider has revoked the saved permission. Do not keep
                // asking repeatedly with a dead URI; clear it and let the user
                // choose a folder when they actually want to.
                prefs.edit().remove(PREF_TREE_URI).apply();
                return;
            }
                ...
                try (Cursor cursor = getContentResolver().query(
                    ...
                    if (cursor != null) {
                        ...
                        while (cursor.moveToNext()) {
                            ...
                            String title = parsed.optString("title", name);
                            String artist = parsed.optString("artist", "Unknown Artist");
                            String album = "Local Music";

                            MediaInfo mediaInfo = readMediaInfo(childUri, title, artist, album);

                            song.put("title", mediaInfo.title);
                            song.put("artist", mediaInfo.artist);
                            song.put("album", mediaInfo.album);

                            if (mediaInfo.artwork != null && !mediaInfo.artwork.isEmpty()) {
                                song.put("artwork", mediaInfo.artwork);
                            } else {
    ...
    private MediaInfo readMediaInfo(
        ...
        try {
            ...
            String metadataTitle = retriever.extractMetadata(
            ...
            String metadataArtist = retriever.extractMetadata(
                    MediaMetadataRetriever.METADATA_KEY_ARTIST
            );
            if (metadataArtist != null && !metadataArtist.trim().isEmpty()) {
                artist = metadataArtist.trim();
            }

            String metadataAlbum = retriever.extractMetadata(
                    MediaMetadataRetriever.METADATA_KEY_ALBUM
            );
            if (metadataAlbum != null && !metadataAlbum.trim().isEmpty()) {
